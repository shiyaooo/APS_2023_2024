...
and print_def def =
  ...
  |ProcDef(id, args, bk) -> "proc(" ^ id ^ ",[" ^ (print_args args) ^ "]," ^ (print_block bk) ^ ")"
  |ProcRecDef(id, args, bk) -> "procrec(" ^ id ^ ",[" ^ (print_args args) ^ "]," ^ (print_block bk) ^ ")"
...
and print_stat s =
  match s with
    ...
    |Call(id, es) -> "call(" ^ id ^ ",[" ^ (print_exprs es) ^ "])"
