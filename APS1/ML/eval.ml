...
and eval_instr (env, mem, flux) stat =
  ...
  |Call (id, es) -> let p = getEnv env id and
                      vals = (List.map (fun e ->eval_expr (env, mem) e) es) in
                    match p with
                    |InP(bk, xs_p, env_p) -> 
...
and eval_defs (env,mem) def =
  ...
  |VarDef(x, _)-> let (a,new_mem) = alloc_mem mem in 
                    ((x,InA(a))::env,new_mem)
  |ProcDef(x, args, bk) -> ((x,InP(bk, eval_args args, env))::env,mem)
  |ProcRecDef(x, args, bk) -> ((x, InPR(bk,x,eval_args args, env))::env,mem)
...
and eval_prog p = 
  (* let (_, flux) = eval_block (env0, [], []) p in flux *)
